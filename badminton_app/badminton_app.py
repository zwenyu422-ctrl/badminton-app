...
def inject_global_css():
    ...
    <style>
        ...
        /* 针对各个数据的特殊颜色 */
        .val-winrate { color: #28a745; } /* 绿色 */
        .val-ability { color: #007BFF; } /* 蓝色 */
        .val-champ { color: #FFC107; }   /* 金色 */
        .val-rank { color: #FF5722; }    /* 橙色 */
        
        /* 列表行样式 */
        .rank-row {
            display: flex; justify-content: space-between; align-items: center;
            padding: 15px; background: white; border-radius: 10px;
            box-shadow: 0 1px 5px rgba(0,0,0,0.05); margin-bottom: 10px; border: 1px solid #eee;
        }
    </style>
    ...
    if os.path.exists(PEAK_FILE):
        with open(PEAK_FILE, 'r', encoding='utf-8') as f:
            ...
            for k, v in data.items():
                if isinstance(v, int):
                    data[k] = {"points": v, "played": v, "won": v, "champs": 0} # 假定以前赢的都算场次
                    upgraded = True
            if upgraded:
                with open(PEAK_FILE, 'w', encoding='utf-8') as fw:
...
if 'stage' not in st.session_state:
    ...
    st.session_state.viewing_player = None # 记录当前正在查看哪个选手主页
...
if st.session_state.stage == 'splash':
    ...
    if os.path.exists(poster_path): st.image(poster_path, use_container_width=True)
    else: st.markdown("<br><br><h1 style='text-align:center;'>🏸<br>王者轮回</h1><br><br>", unsafe_allow_html=True)
...
# ================= 选手个人主页 (独立视图) =================
    ...
    win_rate = "0.0%"
    if p_data["played"] > 0:
        win_rate = f"{(p_data['won'] / p_data['played']) * 100:.1f}%"
    ...
    for ext in ['jpg', 'png', 'jpeg']:
        ...
        if os.path.exists(path):
            avatar_html = f"<img src='data:image/jpeg;base64,{st.image(path)._repr_html_()}' class='profile-avatar-img'>" # 简单占位，Streamlit处理本地图片更推荐直接渲染
            st.markdown("<div class='profile-header'>", unsafe_allow_html=True)
            cols = st.columns([1,2,1])
            with cols[1]:
                st.image(path, use_container_width=True, output_format="auto") # 用内置组件居中渲染最稳
            avatar_found = True
            break
    ...
    with c1:
        st.markdown(f"<div class='stat-card'><div class='stat-val val-winrate'>🎯<br>{win_rate}</div><div class='stat-label'>总胜率</div></div>", unsafe_allow_html=True)
    with c2:
        st.markdown(f"<div class='stat-card'><div class='stat-val val-ability'>⚡<br>{p_data['points']}</div><div class='stat-label'>能力值</div></div>", unsafe_allow_html=True)
    with c3:
        st.markdown(f"<div class='stat-card'><div class='stat-val val-champ'>🏆<br>{p_data['champs']}</div><div class='stat-label'>周赛冠军</div></div>", unsafe_allow_html=True)
    with c4:
        st.markdown(f"<div class='stat-card'><div class='stat-val val-rank'>🔥<br>No.{rank}</div><div class='stat-label'>巅峰榜</div></div>", unsafe_allow_html=True)
    
    st.write("---")
    if st.button("⬅️ 返回大厅", use_container_width=True):
        st.session_state.stage = 'register' # 或者其他状态
        st.rerun()
        ...
        # --- 3. 正赛计分板 ---
            ...
            if st.session_state.p1_wins == 2 or st.session_state.p2_wins == 2:
                ...
                if st.button("⏹️ 结算本局", use_container_width=True):
                    ...
                    if w_code == 0:
                        st.warning("⚠️ 正赛需达21分且净胜2分 (或达30分)。")
                    else:
        ...
        # --- 4. 结算 ---
            ...
            if sorted_st[0][1] == 2:
                ...
                if st.button("🏆 颁发奖杯并重置系统", use_container_width=True, type="primary"):
                    record_championship(champion) # 冠军专属：积分+3，周冠军数+1
        ...
        # --- 6. 加赛计分板 ---
            ...
            if st.session_state.p1_wins == win_req or st.session_state.p2_wins == win_req:
                ...
                if st.button("↩️ 返回加赛大厅", type="primary", use_container_width=True):
                    record_match(winner, loser) # 加赛胜负同样计入总场次和胜率！
